...
async function run() {
  ...
  for (const p of PALETTES) {
    ...
    console.log('Esperando compilación de Next.js (8s)...');
    await new Promise(r => setTimeout(r, 8000));

    console.log(`Tomando captura de ${p.name}...`);
    // load instead of networkidle due to nextjs hmr websockets
    try {
      await page.goto('http://localhost:3001', { waitUntil: 'load', timeout: 30000 });
      await page.reload({ waitUntil: 'load', timeout: 30000 }); 
    } catch(e) {
      console.log('Ignorando timeout de load', e.message);
    }

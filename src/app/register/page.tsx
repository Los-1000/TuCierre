...
function RegisterForm() {
    ...
    return (
      <div className="min-h-screen bg-parchment flex flex-col items-center justify-center px-4 font-sans">
        <div className="w-full max-w-sm text-center">
          ...
          <h2 className="font-display text-3xl font-semibold text-ink mb-2">¡Casi listo!</h2>
  ...
  return (
    <div className="min-h-screen flex font-sans">
      {/* Left brand panel */}
      <div className="hidden lg:flex w-[38%] flex-col bg-brand-navy noise-overlay relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-b from-brand-navy-mid to-brand-navy" />
        <div
        ...
        />
        <div className="absolute top-1/3 right-0 w-64 h-64 bg-brand-gold/10 rounded-full blur-3xl pointer-events-none" />
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={p} className="flex items-start gap-2.5">
                  <Check size={12} className="text-brand-gold mt-0.5 shrink-0" />
      ...
      <div className="flex-1 bg-parchment overflow-y-auto">
        <div className="min-h-full flex flex-col items-center justify-center px-6 py-10">
          ...
          <div className="w-full max-w-md">
            <div className="mb-7">
              <h2 className="font-display text-3xl font-semibold text-ink">Crea tu cuenta</h2>
            ...
            <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2">
                  <Label htmlFor="full_name" className="text-sm font-medium text-ink/80">
                    Nombre completo <span className="text-destructive">*</span>
                  </Label>
                  <Input id="full_name" type="text" placeholder="Juan García Pérez" autoComplete="name"
                    className="mt-1.5 bg-white border-border focus:border-brand-gold h-10" {...register('full_name')} />
                  {errors.full_name && <p className="text-destructive text-xs mt-1">{errors.full_name.message}</p>}
                </div>
                <div>
                  <Label htmlFor="dni" className="text-sm font-medium text-ink/80">
                    DNI <span className="text-destructive">*</span>
                  </Label>
                  <Input id="dni" type="text" placeholder="12345678" maxLength={8} inputMode="numeric"
                    className="mt-1.5 bg-white border-border focus:border-brand-gold h-10 font-mono" {...register('dni')} />
                ...
                <div>
                  ...
                  <Input id="phone" type="tel" placeholder="987 654 321" autoComplete="tel"
                    className="mt-1.5 bg-white border-border focus:border-brand-gold h-10" {...register('phone')} />
              ...
              <div>
                ...
                <Input id="email" type="email" placeholder="tu@email.com" autoComplete="email"
                  className="mt-1.5 bg-white border-border focus:border-brand-gold h-10" {...register('email')} />
                {errors.email && <p className="text-destructive text-xs mt-1">{errors.email.message}</p>}
              </div>

              {/* Company (optional) */}
              <div>
                <Label htmlFor="company_name" className="text-sm font-medium text-ink/80">
                ...
                <Input id="company_name" type="text" placeholder="Inmobiliaria XYZ S.A.C."
                  className="mt-1.5 bg-white border-border focus:border-brand-gold h-10" {...register('company_name')} />
              </div>

              {/* Referral code */}
              <div>
                <Label htmlFor="referral_code" className="text-sm font-medium text-ink/80 flex items-center gap-1.5">
                ...
                <div className="relative mt-1.5">
                  <Input id="referral_code" type="text" placeholder="TC-ABC123"
                    className={cn(
                      'bg-white border-border focus:border-brand-gold h-10 font-mono uppercase',
                      refCode && 'border-brand-gold/50 bg-amber-50/50'
                    )}
                    {...register('referral_code')}
                  />
                  {refCode && (
                    <Check size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-brand-gold" />
                  )}
                </div>
                {refCode && (
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <div className="relative mt-1.5">
                    <Input id="password" type={showPassword ? 'text' : 'password'} placeholder="Mín. 8 caracteres"
                      ...
                      className="bg-white border-border focus:border-brand-gold h-10 pr-9" {...register('password')} />
                ...
                <div>
                  <Label htmlFor="confirm_password" className="text-sm font-medium text-ink/80">
                  ...
                  <div className="relative mt-1.5">
                    <Input id="confirm_password" type={showConfirm ? 'text' : 'password'} placeholder="Repite"
                      autoComplete="new-password"
                      className="bg-white border-border focus:border-brand-gold h-10 pr-9" {...register('confirm_password')} />
              ...
              <Button type="submit" disabled={loading}
                className="w-full h-11 bg-brand-navy hover:bg-brand-navy-light text-parchment font-medium gap-2">
                {loading
            ...
            <p className="text-center text-sm text-muted-foreground mt-5">
              ¿Ya tienes cuenta?{' '}
              <Link href="/login" className="text-brand-gold font-medium hover:text-brand-gold-light transition-colors">
                Iniciar sesión
              </Link>
            </p>
          </div>

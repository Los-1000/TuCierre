...
export default function PersonasSection() {
  return (
    <section id="beneficios" className="py-24 sm:py-32 relative bg-[#FFFEF5]">
      {/* Top subtle divider */}
        ...
        <Reveal>
          <div className="mb-16 text-center max-w-2xl mx-auto">
            <div className="inline-flex items-center gap-2 border border-[#C9880E]/20 bg-[#C9880E]/5 rounded-full px-3 py-1 mb-6">
              <span className="text-[11px] text-[#C9880E] font-semibold tracking-widest uppercase">Para quién</span>
            </div>
            <h2 className="font-display font-semibold leading-tight" style={{ fontSize: 'clamp(36px, 5vw, 56px)', color: '#12161F' }}>
              Diseñado estructuralmente<br/>
              <span className="italic text-[#C9880E]">para ti.</span>
            </h2>
          </div>
        </Reveal>
            ...
            <Reveal key={p.title} delay={i * 100}>
              <div
                className={cn(
                  ...
                  i === 1
                    ? 'bg-[#1B5E4B] border-[#1B5E4B] shadow-[0_16px_40px_rgba(27,94,75,0.25)] relative overflow-hidden'
                    : 'bg-white border-[#12161F]/10 hover:border-[#12161F]/20 shadow-sm'
                )}
              >
                {/* Glow effect for featured card */}
                {i === 1 && (
                  <div className="absolute -top-10 -right-10 w-40 h-40 bg-white/10 blur-[50px] rounded-full pointer-events-none" />
                )}
                ...
                <p
                  ...
                  style={{ color: i === 1 ? '#FFFEF5' : '#12161F', opacity: i === 1 ? 0.9 : 0.65 }}
                >
                  {p.desc}
                </p>
                ...
                <ul className="space-y-4 relative z-10">
                  {p.bullets.map(b => (
                    <li key={b} className="flex items-start gap-3 text-[14px] font-medium" style={{ color: i === 1 ? '#FFFEF5' : '#12161F', opacity: i === 1 ? 1 : 0.8 }}>
                      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" className="shrink-0 mt-0.5">
                        <circle cx="12" cy="12" r="12" fill={i === 1 ? '#FFFEF5' : '#C9880E'} fillOpacity={i === 1 ? "0.2" : "0.1"} />
                        <path d="M8 12l3 3 5-6" stroke={i === 1 ? '#FFFEF5' : '#C9880E'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                      </svg>
                      {b}
                    </li>
                  ))}

...
export default function PersonasSection() {
  return (
    <section id="beneficios" className="py-24 sm:py-32 relative bg-[#F9F9F8]">
      {/* Top subtle divider */}
        ...
        <Reveal>
          <div className="mb-16 text-center max-w-2xl mx-auto">
            ...
            <h2 className="font-display font-semibold leading-tight" style={{ fontSize: 'clamp(36px, 5vw, 56px)', color: '#18181B' }}>
              Hecho para ti.<br/>
              <span className="italic text-[#D47151]">No para la notaría.</span>
            </h2>
          </div>
        </Reveal>
              ...
              <div
                className={cn(
                  ...
                  i === 1
                    ...
                    : 'bg-[#F9F9F8] border-[#18181B]/10 hover:border-[#18181B]/20 shadow-sm'
                )}
              >
                {/* Glow effect for featured card */}
                {i === 1 && (
                  <>
                    <div className="absolute -top-10 -right-10 w-40 h-40 bg-[#D47151]/15 blur-[50px] rounded-full pointer-events-none" />
                    <div className="absolute top-0 inset-x-0 h-px bg-gradient-to-r from-transparent via-[#D47151]/30 to-transparent" />
                  </>
                )}
                
                <h3
                  className="font-display text-[26px] font-semibold mb-4 relative z-10"
                  style={{ color: i === 1 ? '#F9F9F8' : '#18181B' }}
                >
                  {p.title}
                </h3>
                <p
                  className="text-[15px] font-medium leading-relaxed mb-8 relative z-10 flex-grow"
                  style={{ color: i === 1 ? '#F9F9F8' : '#18181B', opacity: i === 1 ? 0.8 : 0.6 }}
                >
                  {p.desc}
                </p>
                ...
                <ul className="space-y-4 relative z-10">
                  {p.bullets.map(b => (
                    <li key={b} className="flex items-start gap-3 text-[14px] font-medium" style={{ color: i === 1 ? '#F9F9F8' : '#18181B', opacity: i === 1 ? 0.9 : 0.7 }}>
                      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" className="shrink-0 mt-0.5">
                        <circle cx="12" cy="12" r="12" fill={i === 1 ? '#D47151' : '#D47151'} fillOpacity={i === 1 ? "0.2" : "0.08"} />
                        <path d="M8 12l3 3 5-6" stroke={i === 1 ? '#D47151' : '#D47151'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                      </svg>
                      {b}
                    </li>
                  ))}

...
function TramiteCard() {
  return (
    <div
      className="rounded-2xl overflow-hidden select-none border border-white/12"
      style={{
        background: '#fff',
    ...
    >
      <div className="px-5 py-4 flex items-center justify-between bg-brand-navy">
        ...
        <span className="text-[11px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full bg-brand-blue/20 text-white/65 border border-white/12">
          En firma
        </span>
      </div>
      ...
      <div className="px-5 py-5 space-y-4">
        {STEPS.map((step) => (
          <div key={step.label} className="flex items-center gap-3">
            ...
            {step.state === 'pending' && (
              <div className="w-5 h-5 rounded-full border-2 shrink-0 border-white/12" />
            )}
      ...
      <div className="px-5 py-3 flex items-center gap-2 border-t bg-brand-navy border-white/12">
